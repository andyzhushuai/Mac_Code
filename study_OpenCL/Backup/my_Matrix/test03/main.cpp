...
//                 -13, -14, -15, -16
    ...
    if (cnt != CL_SUCCESS) 
    {
        cout <<"something wrong with getplatformIDs" << endl;
        return -1;
        }
    ...
    cl_mem a = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, maxn*sizeof(cl_float), arr, &cnt);
    cl_mem b = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, maxn*sizeof(cl_float), brr, &cnt);
    cl_mem c = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(cl_float), NULL, &cnt);
    ...
    cl_event ev;
    size_t globalsz[2] = {1, 1};
    clEnqueueNDRangeKernel(queue, kernel, 2, NULL, globalsz, NULL, 0, NULL, &ev);
    cnt = clFinish(queue);
    clWaitForEvents(1,&ev);

    clEnqueueReadBuffer(queue, c, CL_TRUE, 0, sizeof(cl_uint), crr, 0, NULL, NULL);
cout << *crr <<" "<<endl;
